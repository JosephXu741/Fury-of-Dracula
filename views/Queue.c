...
typedef struct QueueOrgNode {
	Item value;
	struct QueueOrgNode *next;
} QueueOrgNode;

typedef struct QueueOrgRep {
	QueueOrgNode *head; // ptr to first node
	QueueOrgNode *tail; // ptr to last node
} QueueOrgRep;

// create new empty QueueOrg
QueueOrg newQueueOrg (void)
{
	QueueOrgRep *new = malloc (sizeof *new);
	*new = (QueueOrgRep){ .head = NULL, .tail = NULL };
	return new;
}

// free memory used by QueueOrg
void dropQueueOrg (QueueOrg Q)
{
	assert (Q != NULL);
	for (QueueOrgNode *curr = Q->head, *next; curr != NULL; curr = next) {
		next = curr->next;
		free (curr);
	}
	free (Q);
...
// display as 3 > 5 > 4 > ...
void showQueueOrg (QueueOrg Q)
{
	assert (Q != NULL);

	for (QueueOrgNode *curr = Q->head; curr != NULL; curr = curr->next) {
		ItemShow (curr->value);
		if (curr->next != NULL)
			printf (">");
	}
...
// add item at end of QueueOrg
void QueueOrgJoin (QueueOrg Q, Item it)
{
	assert (Q != NULL);

	QueueOrgNode *new = malloc (sizeof *new);
	assert (new != NULL);
	*new = (QueueOrgNode){ .value = ItemCopy (it), .next = NULL };
...
// remove item from front of QueueOrg
Item QueueOrgLeave (QueueOrg Q)
{
	assert (Q != NULL);
	assert (Q->head != NULL);
	Item it = ItemCopy (Q->head->value);
	QueueOrgNode *old = Q->head;
	Q->head = old->next;
	if (Q->head == NULL)
		Q->tail = NULL;
	free (old);
...
// check for no items
int QueueOrgIsEmpty (QueueOrg Q)
{
	return (Q->head == NULL);
}